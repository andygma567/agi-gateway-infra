...
description: Deploys LiteLLM and nginx onto a single Ubuntu VM using this repository's Ansible playbook. Use when the user supplies a VM IP and asks to deploy, provision, re-run, or verify the gateway, or when debugging site.yml, the inventory, or the LiteLLM health endpoint.
---

# Deploy the gateway VM

Bring up LiteLLM + nginx on one Ubuntu VM via Ansible. The human usually supplies only a **VM IP**. `README.md` at the repo root is the runbook; this skill is the operating procedure around it.
...
5. **Check health.** If you cannot edit `/etc/hosts`, set the `Host` header by hand instead, which is what nginx routes on:

   ```bash
   curl -si -H 'Host: litellm.test'  http://VM_IP/health/liveliness | head -1
   ```

6. **Report** the playbook recap, the health check status code, and the UI URL plus default login from `README.md`.
...
- Change LiteLLM secrets unless asked.
...
- LiteLLM listens on host port **4000**, nginx on **80**.
- Hostname is `litellm.test`. `.test` is a domain suffix like `.com`, not a test suite.
